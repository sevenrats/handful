...
func (a *Dialer) dial(ctx context.Context) (*ClientConn, error) {
	...
	if a.SRVDiscovery {
		a.logf("controlhttp: SRV discovery enabled for host %q", a.Hostname)
		srvCands, err := lookupControlSRV(ctx, a.Hostname, a.logf, nil)
		if err != nil {
			a.logf("controlhttp: SRV discovery for %q failed: %v", a.Hostname, err)
		} else if len(srvCands) > 0 {
			a.logf("controlhttp: merging %d SRV candidate(s) into dial plan", len(srvCands))
			candidates = append(candidates, srvCands...)
		} else {
			a.logf("controlhttp: SRV discovery for %q returned no candidates", a.Hostname)
		}
	} else {
		a.logf("[v1] controlhttp: SRV discovery is disabled")
	}
